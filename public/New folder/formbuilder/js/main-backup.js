...
function addElement(pageid,elementtype,datatypeid,elementdata){
    ...
    if( elementdata ){
        $("#question_"+elementdata.questionDets.question_pool_uuid).hide();
        //$(typeHTML).insertAfter($("#question_"+elementdata.questionDets.question_pool_uuid));
        ...
        //getPageQuestions(1,elementdata.questionDets.question_pool_uuid);
    }else{
        $("#form-page-"+pageid).children('.page-content').append(typeHTML);
...
function getTypeHTML(elementtype,pageid,datatypeid,elementdata){
    ...
    var elementValue = elementkey = sign1Text = sign2Text = witnessText = qCodeValue = descValue = scoredYesSel = scoredNoSel = '';
    ...
    if( elementdata ){
        elementValue = elementdata.questionDets.question;
        ...
        elementkey = elementdata.questionDets.question_pool_uuid;
    ...
    qHTML +='<div class="fake-element-wrapper p-4">';
    ...
    if( elementtype == 'statictext' ){
            ...
            '<div class="row">'+
                '<div class="col-12 col-md-12 mb-3">'+
                    '<div class="form-group">'+
                        '<label for="questionname_'+pageid+'">Question<span class="red">*</span></label>'+
                        '<input type="text" class="form-control" placeholder="Enter your question" name="question" id="questionname_'+pageid+'" value="'+elementValue+'" />'+
                    '</div>'+
                '</div>';
        
                if( moduleurl != 'test' && moduleurl != 'evaluation' ){
                    qHTML += '<div class="col-12 col-md-3 mb-3">'+
                        '<div class="form-group">'+
        ...
        if( elementtype == 'singlechoice' ){
			qHTML += '<div style="border-radius: 6px;border: none;background: #ecf8f8;" class="col-12 px-3 dragcls_'+pageid+'"> ';
            var i;
            for (i = 1; i <= elemChoiceCnt; i++) {
              ...
              qHTML += '<div class="row multisort_drag choices_'+pageid+'" id="form-choice_'+pageid+'_'+i+'"><div class="col-8"><div class="form-group mt-2 form-check element-choices-'+pageid+'"><input type="radio" disabled class="form-check-input" style="margin-top: 12px;"><input type="text" class="form-control choice-inps pl-2" placeholder="Enter your choice" name="choices['+i+'][choice]" value="'+choiceValue+'" >'+choiceHiddenHTML+'</div></div><div class="col-2 text-center"><div class="form-group mt-3"><label for="rightanswer">Right Answer</label><label class=""><div class="form-check pl-2"><input class="form-radio-input" type="radio" name="rightanswer" value="'+i+'" '+choiceSel+' ></div></label></div></div><div class="col-2 text-right"><a class="btn btn-sm btn-outline-secondary mt-2 icondrag" style="margin-right: 2px;"><i class="fas fa-bars"></i></a><a href="javascript:void(0)" onclick="addChoice(\''+pageid+'\',\''+elementtype+'\')" style="margin-right: 2px;" class="btn btn-sm btn-outline-primary mt-2"><i class="fa fa-plus"></i></a><a href="javascript:void(0)" onclick="removeChoice(\''+pageid+'\',\''+i+'\')" class="btn btn-sm btn-outline-danger mt-2"><i class="fa fa-minus"></i></a></div></div>';
            }
        ...
        if( elementtype == 'ranking' ){
            var rankType = 'row';
			qHTML += '<div style="border-radius: 6px;border: none;background: #ecf8f8;" class="px-3 col-12 dragcls_'+rankType+'_'+pageid+'"><div class="col-12"><strong>Row Choices</strong></div> ';
            var i;
            for (i = 1; i <= elemRowCnt; i++) {
              ...
              qHTML += '<div class="col-2 text-right"><a class="btn btn-sm btn-outline-secondary mt-2 icondrag" style="margin-right: 2px;"><i class="fas fa-bars"></i></a><a href="javascript:void(0)" onclick="addChoice(\''+pageid+'\',\''+elementtype+'\',\''+rankType+'\')" style="margin-right: 2px;" class="btn btn-sm btn-outline-primary mt-2"><i class="fa fa-plus"></i></a><a href="javascript:void(0)" onclick="removeChoice(\''+pageid+'\',\''+i+'\',\''+rankType+'\')" class="btn btn-sm btn-outline-danger mt-2"><i class="fa fa-minus"></i></a></div></div>';
            }
			 qHTML += '<div class="col-9 tr errordiv" id="righterror_'+rankType+'_'+pageid+'"></div><div class="col-3"></div></div>';
            
            
            var rankType = 'column';
			qHTML += '<div style="border-radius: 6px;border: none;background: #ecf8f8;" class="px-3 col-12 dragcls_'+rankType+'_'+pageid+'"><div class="col-12"><strong>Column Choices</strong></div> ';
            var i;
...
function addChoice(pageid,elementtype,rankType){
    ...
    if( elementtype == 'ranking' ){
        ...
        choiceHTML += '<div class="col-2 text-right"><a class="btn btn-sm btn-outline-secondary mt-2 icondrag" style="margin-right: 2px;"><i class="fas fa-bars"></i></a><a href="javascript:void(0)" onclick="addChoice(\''+pageid+'\',\''+elementtype+'\',\''+rankType+'\')" style="margin-right: 2px;" class="btn btn-sm btn-outline-primary mt-2"><i class="fa fa-plus"></i></a><a href="javascript:void(0)" onclick="removeChoice(\''+pageid+'\',\''+choiceCount+'\',\''+rankType+'\')" class="btn btn-sm btn-outline-danger mt-2"><i class="fa fa-minus"></i></a></div></div>';
    }
